...
# Observation Model 3: PO, MA PO, MD PO, ME PO, NY Atlas, VT PO, WV PO
# 51266 cells
for (j in 1:nW3) {
...
# 167 observations, 1 median visits per site
for (j in 1:nY5) {
...
# 1195 observations, 1 median visits per site
for (j in 1:nY6) {
...
# Observation Model 7: count, SE Stream VES
# 14 observations, 1 median visits per site
for (j in 1:nY7) {
...
# 262 observations, 1 median visits per site
for (j in 1:nY8) {
...
# Observation Model 9: count, Dodd Litterbag
...
# Observation Model 10: count, Dodd Terrestrial
...
# Observation Model 11: count, Hyde
# 922 observations, 9 median visits per site
for (j in 1:nY11) {
  ...
  # Observation model
  Y11[j] ~ dbinom(p11, ND11[j]) # N-mixture
...
# Observation priors, PO 3: MA PO, MD PO, ME PO, NY Atlas, VT PO, WV PO
for (b in 1:nCovW3) {
  A3[b] ~ dnorm(0,1)
}
...
# Observation priors, count 7: SE Stream VES
for (b in 1:nCovY7) {
  C7[b] ~ dnorm(0,1)
}
...
# Observation priors, count 9: Dodd Litterbag
for (b in 1:nCovY9) {
  C9[b] ~ dnorm(0,1)
}

# Observation priors, count 10: Dodd Terrestrial
for (b in 1:nCovY10) {
  C10[b] ~ dnorm(0,1)
}

# Observation priors, count 11: Hyde
for (b in 1) {
  C11[b] ~ dnorm(0,1)
}
