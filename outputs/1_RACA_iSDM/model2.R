...
# Observation Model 3: PO, BLM Surveys, NCCN, ORBIC, WDWF
# 14502 cells
for (j in 1:nW3) {
...
# 78 observations, 2 median visits per site
for (j in 1:nV4) {
...
# Observation Model 5: count, ARMI Count
# 374 observations, 4 median visits per site
for (j in 1:nY5) {
  ...
  # Observation model
  Y5[j] ~ dbinom(p5[j], ND5[j]) # N-mixture

  # Detection 
  logit(p5[j]) <- inprod(C5[1:nCovY5], Xy5[j,1:nCovY5])
...
# Observation Model 6: count, USGS RACA assessment - current
# 56 observations, 1 median visits per site
for (j in 1:nY6) {
...
# Observation Model 7: count, PNW Salamanders
# 71 observations, 2 median visits per site
for (j in 1:nY7) {
...
# Observation priors, PO 1: Museum
for (b in 1:nCovW1) {
  A1[b] ~ dnorm(0,1)
}
...
# Observation priors, PO 3: BLM Surveys, NCCN, ORBIC, WDWF
for (b in 1:nCovW3) {
  A3[b] ~ dnorm(0,1)
}
...
# Observation priors, count 5: ARMI Count
for (b in 1:nCovY5) {
  C5[b] ~ dnorm(0,1)
}

# Observation priors, count 6: USGS RACA assessment - current
for (b in 1:nCovY6) {
  C6[b] ~ dnorm(0,1)
}

# Observation priors, count 7: PNW Salamanders
for (b in 1:nCovY7) {
  C7[b] ~ dnorm(0,1)
}
