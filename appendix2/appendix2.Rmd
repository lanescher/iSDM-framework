...
![Example of data structure](annotateddata.png)
...
![Distribution of species data across the subrange](../outputs/3_RACA_subrange/2_inputmap-a_data-full.jpg)
...
![Distribution of data with overlaid cross-validation blocks](../outputs/3_RACA_subrange/2_inputmap-c_blocks-full.jpg)
...
if (fold.out == "none") {
  ...
  if ("iNaturalist" %in% names(species.data$obs)) {
    ...
    out <- plot_covar(covar,
                      ...
                      scaled = T)
    ggsave(out$plot, height = 12, width = 12, 
           file = paste0(out.dir, "1_covariates-b_POinat-map.jpg"))
    ...
    if (length(covs.inat) > 1) {
      out <- cor_covar(covar,
                       ...
                       color.threshold = 0.25)
      ggsave(out$plot, height = 12, width = 12, 
             file = paste0(out.dir, "1_covariates-b_POinat-cors.jpg"))
    }
  }
  ...
  out <- plot_covar(covar,
                    ...
                    scaled = T)
  ggsave(out$plot, height = 12, width = 12, 
         file = paste0(out.dir, "1_covariates-c_PO-map.jpg"))
  ...
  if (length(covs.PO) > 1) {
    out <- cor_covar(covar,
                     ...
                     color.threshold = 0.25)
    ggsave(out$plot, height = 12, width = 12, 
           file = paste0(out.dir, "1_covariates-c_PO-cors.jpg"))
  }
}
```


![Distribution of distribution-level covariates across the subrange](../outputs/3_RACA_subrange/1_covariates-a_process-map.jpg)

![Correlation between distribution-level covariates](../outputs/3_RACA_subrange/1_covariates-a_process-correlations.jpg)

![Distribution of presence-only (PO) effort covariates](../outputs/3_RACA_subrange/1_covariates-c_PO-map.jpg)
...
mods <- read.csv("code/model-specs.csv") %>% 
  filter(number %in% num) # get species code and model name from csv file
sp.code <- mods$sp.code[1] # species code
model <- mods$model[1] # model name
...
First, view the chains and convergence metrics to assess whether the chains are sufficiently long.
```{r plot-sum-output}
# Read in labels that will be used for all output figures
cov.labs <- read.csv("data/covariate-labels.csv")


# Convergence metrics for all chains
outplot <- plot_convergence(out)
...
outplot <- plot_posteriors(samples, data = data, cov.labs = cov.labs,
                           plot = "B", cutoff = 0)
ggsave(outplot$plot,
       file = paste0(out.dir, "3_parameters-a2_posteriors-B.jpg"),
       height = 6, width = 8)

# Alpha (dataset intercepts)
outplot <- plot_chains(samples, data = data, constants = constants, 
                       cov.labs = cov.labs, plot = "alpha", cutoff = 0)
ggsave(outplot$plot,
       file = paste0(out.dir, "3_parameters-b1_chains-alpha.jpg"),
       height = 6, width = 8)

outplot <- plot_posteriors(samples, data = data, constants = constants, 
                           cov.labs = cov.labs, plot = "alpha", cutoff = 0)
ggsave(outplot$plot,
...
# Tau - we fixed tau at 1, but tau could be estimated
outplot <- plot_chains(samples, data = data, cov.labs = cov.labs,
                       plot = "tau", cutoff = 0)
ggsave(outplot$plot,
...
Generally, an Rhat below 1.05 indicates that the chains have fully mixed and the results can be interpreted. We ran relatively short chains to minimize the computational load, so we ended up with higher Rhat. For the sake of practice, we'll look at the results anyway.

We can plot the estimated parameters `plot_pars()`. Additionally, the marginal effects of the distribution model parameters can be visualized with `plot_effects()`.

```{r plot-parameters}
outplot <- plot_pars(out$process.coef,
                     plot.type = "full",
                     cov.labs = cov.labs)
ggsave(outplot$plot,
...
outplot <- plot_effects(data, out, cov.labs = cov.labs, breaks = 0.001)
...
outplot <- plot_pars(out$alpha,
                     plot.type = "full")
ggsave(outplot$plot,
       file = paste0(out.dir, "3_parameters-b3_alpha.jpg"),
       height = 6, width = 10)

outplot <- plot_pars(out$obs.coef,
                     cov.labs = cov.labs,
                     plot.type = "full")
ggsave(outplot$plot,
...
Finally, the `map_species_data()` function can be used to plot the model output. There are a variety of ways to customize the maps.

Set up the map with:
- `title`
- `region`
- `plot` (options are 'samples', 'lambda', 'psi', 'boundary', 'effort', 'XB' or 'spat')

Features can be added to the map using:
- `plot.range` (default T)
- `plot.region` (default F)
- `plot.cells` (default F)

Output is added to the map using:
- `out` contains the model output
- `plot.current` (default is T, use F to plot projections)
- `plot.uncertainty` (default is F, can be 'unc.rel' or 'unc.range')
- `transform` (default is 'none', can be 'exp' or 'log')
- `plot.change` (default is F, can be 'relative' or 'absolute'; only used if `plot.current` = F)
- `proj.n` (default is 0, number of projections; only used if `plot.current` = F)
- `proj.names` (only used if `plot.current` = F)
- `threshold` (default is 0.5, threshold to determine presence based on occupancy probability; only used if `plot` = 'boundary')


Here are some examples:
```{r plot-maps}

# A few ways to look at relative abundance
outplot <- map_species_data(title = "Lambda, mean estimate", 
                            region = region, plot = "lambda",
                            out = out,
                            plot.range = F, plot.region = F, plot.cells = F,
                            plot.current = T, plot.uncertainty = F,
                            transform = "none", plot.change = F)

ggsave(outplot$plot, file = paste0(out.dir, "4_mapcurrent-a_lambda.jpg"),
       height = 7, width = 9)

outplot <- map_species_data(title = "Lambda, relative uncertainty", 
                            region = region, plot = "lambda",
                            out = out,
                            plot.range = F, plot.region = F, plot.cells = F,
...
ggsave(outplot$plot, file = paste0(out.dir, "4_mapcurrent-a_lambda-relunc.jpg"),
       height = 7, width = 9)


outplot <- map_species_data(title = "Log(lambda)", 
                            region = region, plot = "lambda",
                            out = out,
                            plot.range = F, plot.region = F, plot.cells = F,
                            plot.current = T, plot.uncertainty = F,
...
ggsave(outplot$plot, file = paste0(out.dir, "4_mapcurrent-a_lambdalog.jpg"),
       height = 7, width = 9)

# Occupancy probability
outplot <- map_species_data(title = "Psi, mean estimate", 
                            region = region, plot = "psi",
                            out = out,
                            plot.range = F, plot.region = F, plot.cells = F,
                            plot.current = T, plot.uncertainty = F,
                            transform = "none", plot.change = F)

ggsave(outplot$plot, file = paste0(out.dir, "4_mapcurrent-b_psi.jpg"),
       height = 7, width = 9)

# Occupied range boundary
outplot <- map_species_data(title = "Boundary, threshold = 0.75", 
                            region = region, plot = "boundary",
                            out = out,
                            plot.range = F, plot.region = F, plot.cells = F,
                            plot.current = T, plot.uncertainty = F,
                            transform = "none", plot.change = F,
...
ggsave(outplot$plot, file = paste0(out.dir, "4_mapcurrent-c_boundary.jpg"),
       height = 7, width = 9)

# Spatial effect
outplot <- map_species_data(title = "Spatial effect, mean estimate", 
                            region = region, plot = "spat",
                            out = out,
                            plot.range = F, plot.region = F, plot.cells = F,
                            plot.current = T, plot.uncertainty = F,
                            transform = "none", plot.change = F)

ggsave(outplot$plot, file = paste0(out.dir, "4_mapcurrent-d_spat.jpg"),
       height = 7, width = 9)

# X * B (i.e., variation in relative abundance from covariates)
outplot <- map_species_data(title = "XB, mean estimate", 
                            region = region, plot = "XB",
                            out = out,
                            plot.range = F, plot.region = F, plot.cells = F,
                            plot.current = T, plot.uncertainty = F,
                            transform = "none", plot.change = F)

ggsave(outplot$plot, file = paste0(out.dir, "4_mapcurrent-e_xb.jpg"),
       height = 7, width = 9)

# Effort
outplot <- map_species_data(title = "Effort, mean estimate", 
                            region = region, plot = "effort",
                            out = out,
                            plot.range = F, plot.region = F, plot.cells = F,
                            plot.current = T, plot.uncertainty = F,
                            transform = "none", plot.change = F)

ggsave(outplot$plot, file = paste0(out.dir, "4_mapcurrent-f_effort.jpg"),
       height = 7, width = 9)

```

![Map of relative abundance (lambda)](../outputs/3_RACA_subrange/4_mapcurrent-a_lambda.jpg)


![Map of relative uncertainty for relative abundance (lambda)](../outputs/3_RACA_subrange/4_mapcurrent-a_lambda-relunc.jpg)


![Map of log relative abundance](../outputs/3_RACA_subrange/4_mapcurrent-a_lambdalog.jpg)


![Map of relative occupancy](../outputs/3_RACA_subrange/4_mapcurrent-b_psi.jpg)

![Map of range boundary defined by relative occupancy threshold](../outputs/3_RACA_subrange/4_mapcurrent-c_boundary.jpg)


![Map of the conditional autoregressive spatial effect](../outputs/3_RACA_subrange/4_mapcurrent-d_spat.jpg)


![Map of the fixed linear effects (X * beta)](../outputs/3_RACA_subrange/4_mapcurrent-e_xb.jpg)

![Map of sampling effort for PO data](../outputs/3_RACA_subrange/4_mapcurrent-f_effort.jpg)



## Calculate and plot projections

At some point you may want to project the covariate relationships onto future conditions. Let's say we want to increase the area of small ponds as a management action and assess the impacts of this potential change. We simulate this action by arbitrarily adding a normally-distributed offset with two different means. We set up the projections in a separate file `app-setup-projections.R` that contains the following lines of code:

```{r, echo = F, results = 'as.is'}

cat(readLines("appendix2/app-setup-projections.R"), sep = "\n")
# file="app-setup-projections.R", 
```

You can always calculate the projections simultaneously with calculating the derived quantitites (e.g., occupancy probability $\psi$) using the `get_derived()` function. We illustrate the workflow below if you additional want to include the two projections.
...
samples <- lapply(samples, get_derived, data = data, project = 2, 
                  coarse.grid = coarse.grid, spatRegion = spatRegion, sp.auto = sp.auto,
                  pathToProj = 'appendix2/app-setup-projections.R')
...
We can additionally plot the estimated relative abundance, relative occupancy probability, associated uncertainties, and absolute/relative changes from current to future estimates. We show a few example plots below:
...
if (fold.out == "none") {
  ...
  ### Future projections ----
  if (project > 0) {
    pl <- map_species_data(title = "Projected lambda, mean estimate",
                           region,
                           plot = "lambda",
                           out = out,
                           plot.current = F,
    ...
    ggsave(pl$plot, file = paste0(out.dir, "5_mapfuture-a_lambda.jpg"),
           height = 7, width = 9)
    
    pl <- map_species_data(title = "Projected lambda, relative uncertainty",
                           region,
                           plot = "lambda",
                           out = out,
                           plot.current = F,
                           ...
                           proj.n = 2,
                           proj.names = proj.names)
    ggsave(pl$plot, file = paste0(out.dir, "5_mapfuture-a_lambda-uncrel.jpg"),
           height = 7, width = 9)
    
    pl <- map_species_data(title = "Projected occupancy probability, mean estimate",
                           region,
                           plot = "psi",
                           out = out,
                           plot.current = F,
                           ...
                           proj.n = 2,
                           proj.names = proj.names)
    ggsave(pl$plot, file = paste0(out.dir, "5_mapfuture-b_psi.jpg"),
           height = 7, width = 9)
    
    
    pl <- map_species_data(title = "Projected XB, mean estimate",
                           region,
                           plot = "XB",
                           out = out,
                           plot.current = F,
                           proj.n = 2,
                           proj.names = proj.names)
    ggsave(pl$plot, file = paste0(out.dir, "5_mapfuture-d_expXB-future.jpg"),
           height = 7, width = 9)
    
    
    ### Future absolute change ----
    pl <- map_species_data(title = "Projected absolute change in lambda",
                           region,
                           plot = "lambda",
                           out = out,
                           plot.current = F,
                           ...
                           proj.n = 2,
                           proj.names = proj.names)
    ggsave(pl$plot, file = paste0(out.dir, "6_mapchange-a1_lambda-abs.jpg"),
           height = 7, width = 9)
    
    pl <- map_species_data(title = "Projected absolute change in psi",
                           region,
                           plot = "psi",
                           out = out,
                           plot.current = F,
                           ...
                           proj.n = 2,
                           proj.names = proj.names)
    ggsave(pl$plot, file = paste0(out.dir, "6_mapchange-b1_psi-abs.jpg"),
           height = 7, width = 9)
    
    ### Future relative change ----
    pl <- map_species_data(title = "Projected relative change in lambda",
                           region,
                           plot = "lambda",
                           out = out,
                           plot.current = F,
                           ...
                           proj.n = 2,
                           proj.names = proj.names)
    ggsave(pl$plot, file = paste0(out.dir, "6_mapchange-a2_lambda-rel.jpg"),
           height = 7, width = 9)
    ...
    pl <- map_species_data(title = "Projected relative change in lambda",
                           ...
                           proj.n = 2,
                           proj.names = proj.names)
    ggsave(pl$plot, file = paste0(out.dir, "6_mapchange-b2_psi-rel.jpg"),
           height = 7, width = 9)
  }
}
```

![Map of projected relative abundance (lambda)](../outputs/3_RACA_subrange/5_mapfuture-a_lambda.jpg)

![Map of relative change in relative abundance from current to future projections](../outputs/3_RACA_subrange/6_mapchange-a2_lambda-rel.jpg)
