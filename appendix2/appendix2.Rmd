...
![Distribution of species data across the subrange](C:/Users/rmummah/OneDrive - DOI/Documents/GitHub/iSDM-framework/appendix2/annotateddata.png)
...
## Save plot
ggsave(pl, file = paste0(out.dir, "2_inputmap-a_data-", foldname, ".jpg"),
       height = 8, width = 10)
```
...
if (fold.out != "none") {
  ...
  pl <- map_species_data(region = region,
                         ...
                         details = F,
                         title = paste0(common, " (", sp.code, ")", title))
  ggsave(pl, file = paste0(out.dir, "2_inputmap-c_blocks-", foldname, ".jpg"),
         height = 8, width = 10)
} else {
  # Plot
  pl <- map_species_data(region = region,
...
rm(list=c('local','fold','args','conus.covar.grid','conus.grid',
...
Because this script can be run independently of `01-flexiSDM.R`, we set some key parameters manually and load the setup file created before proceeding.


```{r param.edit2}
num <- 3 # model number
fold <- "none" # CV fold
sp.code <- "RACA" # 4-digit species code (must match model number row in model-specs.csv)
model <- "subrange" # model name (must match model number row in model-specs.csv)
local <- 1 # are you fitting the model locally (1) or on an HPC (0)?
chain <- 1 # chain number (only relevant if fitting as an array job on an HPC)
```
...
If you intend to fit the model locally, the function `nimbleParallel` will create a local cluster on your computer with a default of 3 cores (this can be changed as an argument to the function). It then configures, compiles, and runs the MCMC in parallel for the number of chains you have defined. The output is then saved as a single output object. 
