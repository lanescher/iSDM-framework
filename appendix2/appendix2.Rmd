...
For the purposes of this demonstration, we're going to fit a model to the centroid of the Cascades frog (RACA) range with a 50,000km buffer. This will allow us to easily visualize the datasets and spatial grid, while maintaining local computational capabilities. You can see all of the specifications for this model in the third row of `mods`.
...
There is one additional step here if you intend to use a coarse spatial grid for the spatial model. Using a coarser grid improves computation time, which may be of concern for large-range species. We must redefine the spatial grid by grouping hexbins together in sets of seven via the `make_spatkey` function. Otherwise, `spatRegion` is set to NULL.
...
We can also take a look at the distribution of data across the cross-validation blocks.
...
We're now ready to download covariate data for the subrange. We use functions from the `SpFut.covariates` package. These functions consist of wrappers for other data sources (e.g., National Hydrologic Database IS THAT WHAT IT'S CALLED? and  the `geodata` R package) and download and format the data for our purposes. See the documentation for the `SpFut.covariates` package for more information on how to use the functions properly.

DO WE WANT TO INCLUDE A DEMONSTRATION OF "OTHER INAT SPECIES" AS A COVARIATE OR SKIP IT IN THIS APPENDIX?

Just a note: Downloading and assembling the covariate data can take significant time depending on the size of the range. We demonstrate how we downloaded the data below but provide the `covariates.rds` file to proceed to the next step.
...
To load the data into NIMBLE, the data must be formatted in a specific way. We have wrapped this process in a function which loads all datasets (`allfiles`), distribution-level covariates, PO and iNat covariates ADD TEXT HERE
